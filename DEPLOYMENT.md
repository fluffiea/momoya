...
> **日常部署**：以根目录 **[README.md](./README.md)** 为准（`/opt/momoya/web`、`git pull`、`deploy/server-update.sh`）。以下为补充参考。

---

> **对象**：单台 Linux 服务器（Ubuntu 22.04+ / Debian 12+ 推荐）
> **目标**：从空机到上线、稳定运行、可备份、可灾备
> **方案**：Docker Compose（mongo + api + web 三容器）。**推荐**：TLS 在独立 **momoya-gateway** 终止，`web` 容器内 nginx **仅监听 80**，网关反代 `momoya.store` → `momoya-web:80`；**api/web 镜像默认从 GHCR 拉取**（GitHub Actions），与默认分支一致。
>
...
```
浏览器 ──HTTPS:443──► momoya-gateway
                       ├─ momoya.store /api/* ──► momoya-web-api:4000
                       └─ momoya.store /      ──► momoya-web:80（仅静态 SPA）
                 web 容器内 nginx 不再反代 /api
                       api ──► mongo
```

- **生产默认**：HTTPS 在网关；`web` 与网关同接 **`momoya-bridge`**。详见根目录 **README.md**。
...
```bash
mkdir -p /opt/momoya/web          # 项目代码
mkdir -p /var/backups/momoya  # 备份归档
chown deploy:deploy /opt/momoya/web /var/backups/momoya
```
...
scp xxxxxx_yourdomain.com_bundle.crt root@<服务器IP>:/opt/momoya/web/deploy/certs/cert.crt
scp xxxxxx_yourdomain.com.key        root@<服务器IP>:/opt/momoya/web/deploy/certs/cert.key
```
...
~/.acme.sh/acme.sh --install-cert -d <你的域名> --ecc \
  --fullchain-file /opt/momoya/web/deploy/certs/cert.crt \
  --key-file       /opt/momoya/web/deploy/certs/cert.key \
  --reloadcmd      "cd /opt/momoya/web && docker compose -f docker-compose.prod.yml restart web"

chmod 600 /opt/momoya/web/deploy/certs/cert.key
```
...
> 所有命令默认在 `/opt/momoya/web` 目录下、用 `deploy` 用户执行。
...
scp deploy@<服务器IP>:/opt/momoya/web/deploy/certs/cert.crt ~/backups/momoya-certs/
scp deploy@<服务器IP>:/opt/momoya/web/deploy/certs/cert.key ~/backups/momoya-certs/
...
```cron
0 3 * * * /opt/momoya/web/scripts/backup.sh >> /var/log/momoya-backup.log 2>&1
```

然后创建脚本：

```bash
mkdir -p /opt/momoya/web/scripts
cat > /opt/momoya/web/scripts/backup.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x /opt/momoya/web/scripts/backup.sh
```
...
把密码管理器里的内容写回 `/opt/momoya/web/.env.production`，记得 `chmod 600`。
...
scp ~/backups/momoya-certs/cert.crt deploy@<新服务器IP>:/opt/momoya/web/deploy/certs/cert.crt
scp ~/backups/momoya-certs/cert.key deploy@<新服务器IP>:/opt/momoya/web/deploy/certs/cert.key
```
...
```bash
nano /opt/momoya/web/.env.production
# 追加：
# MOMOYA_IMAGE_PREFIX=ghcr.io/<你的 GitHub 用户名小写>
# MOMOYA_IMAGE_TAG=latest
```
...
# === 服务器 ===
cd /opt/momoya/web && bash deploy/server-update.sh
...
```bash
sed -i "s/^SESSION_SECRET=.*/SESSION_SECRET=$(openssl rand -hex 32)/" /opt/momoya/web/.env.production
docker compose -f docker-compose.prod.yml up -d
```
...
```bash
sudo chown -R $(whoami):$(whoami) /opt/momoya/web
```
...
```bash
grep -n "^~ /api/" /opt/momoya/web/deploy/nginx.conf
```
...
| 手动备份 | `bash /opt/momoya/web/scripts/backup.sh` |
